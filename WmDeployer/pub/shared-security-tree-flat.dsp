				// Scheduled Tasks
				// serverAlias
				%invoke wm.deployer.gui.UIFlatFile:listScheduledServices%
				...
				// Ports
				// serverAlias
				%invoke wm.deployer.gui.UIFlatFile:listPorts%
				...
				// Users
				// serverAlias
				%invoke wm.deployer.gui.UIFlatFile:listUsers%
					%ifvar users -notempty%
						...
						if (matchRegularExpression("%value /regExp%", "%value userName%")) {
							var p = new WebFXCheckBoxTreeItem("%value userName%", null, false, null, 
								...
								"%value ../serverAlias -urlencode%$%value userName%$USER_INCL", "USER_INCL");
							userTree.add(p);
							matchCnt++;
						}
						else
				...
				// Groups
				// serverAlias
				%invoke wm.deployer.gui.UIFlatFile:listGroups%
					%ifvar groups -notempty%
						...
						if (matchRegularExpression("%value /regExp%", "%value groupName%")) {
							var p = new WebFXCheckBoxTreeItem("%value groupName%", null, false, null, 
								...
								"%value ../serverAlias -urlencode%$%value groupName%$GROUP_INCL", "GROUP_INCL");
							groupTree.add(p);
							matchCnt++;
						}
						else
				...
				// ACLs
				// serverAlias
				%invoke wm.deployer.gui.UIFlatFile:listAcls%
					%ifvar acls -notempty%
				...
				// Extended Settings
				// serverAlias
				%invoke wm.deployer.gui.UIFlatFile:listExtendedSettings%

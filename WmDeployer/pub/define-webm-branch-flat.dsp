<HTML><HEAD><TITLE>Define webMethods Branch</TITLE>
...
<TABLE width="100%">
  <TBODY>
  <TR>
    <TD class=menusection-Deployer>%value bundleName% > webMethods Files > %value serverAliasName%:%value topDir%</TD>
  </TR>
  </TBODY>
</TABLE>
...
<TABLE width="100%">
  <TR>
    <TD colspan="2">
      <UL>
				<LI> <A onclick="return startProgressBar('Refreshing webMethods Files');" href="define-webm-branch-flat.dsp?projectName=%value projectName%&bundleName=%value bundleName%&serverAliasName=%value serverAliasName%&topDir=%value topDir%&regExp=%value regExp%">Refresh this Page</A></LI> 
       </UL>
    </TD>
  </TR>
</TABLE>
...
<TABLE class="tableView" width="100%">
  ...
  <TR>
    ...
    <TD class="heading" valign="top"> Select %value serverAliasName%:%value topDir% Files</TD>
...
%endcomment%
	<FORM id="form" name="form" method="POST" action="define-webm-branch-flat.dsp">
	...
	<TR>
		...
		<INPUT type="hidden" name="topDir" value='%value topDir%'>
...
<script>
	...
	// webMethods Files
	%scope param(recurse='false')%
	%invoke wm.deployer.gui.UIFlatFile:listFiles%

	var backTree = new WebFXTree("..",
%ifvar parentTopDir -notempty%
		"define-webm-branch-flat.dsp?projectName=%value /projectName%&bundleName=%value /bundleName%&serverAliasName=%value ../serverAliasName%&topDir=%value parentTopDir -urlencode%&regExp=%value /regExp%", 
%else%
		"define-webm-files-flat.dsp?projectName=%value /projectName%&bundleName=%value /bundleName%&regExp=%value /regExp%", 
%endif%
				null, webFXTreeConfig.previousIcon, webFXTreeConfig.previousIcon, webFXTreeConfig.linkClass);

	w(backTree);

	var fileTree = new WebFXTree("%value topDir -code%");

		// A 1-pass algorithm works since the folder list is flat
		%loop folders% 
			...
			var fItem = new WebFXCheckBoxTreeItem("%value folderName -code%", 
				...
				"%value ../serverAliasName -urlencode%$DEPLOYERDATA$%value fullFolderName -urlencode%$DEPLOYERDATA$FOLDER_INCL", "FOLDER_INCL", webFXTreeConfig.linkClass);
			fileTree.add(fItem);
		%endloop folders%
		...
		%loop nodes%
			var nItem = new WebFXCheckBoxTreeItem("%value name -code%", null, false, null, 
				...
				"%value ../serverAliasName -urlencode%$DEPLOYERDATA$%value fullName -urlencode%$DEPLOYERDATA$FILE_INCL", "FILE_INCL");
			fileTree.add(nItem);
		%endloop nodes%
